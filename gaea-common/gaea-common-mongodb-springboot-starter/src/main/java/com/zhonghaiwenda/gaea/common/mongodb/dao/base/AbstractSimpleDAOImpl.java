 ...
 * @date 2019/12/18
 */
public abstract class AbstractSimpleDAOImpl<T extends BaseSimpleDO> implements BaseDAO<T, Query, QueryDTO> {

    protected MongoTemplate mongoTemplate;

    /****
     *此属性不得直接使用
     * 必须用getDbName()方法获取  公共内容已经将getDbName变成了切点
     * 同时希望同学们养成好习惯 尊重封装
     * */
    ...
    public void saveOrUpdate(T t) {
        Assert.isTrue(insertCheck(t), "插入异常");
        t.fill();
    ...
    public long update(T t) {
        ...
        return mongoTemplate.updateMulti(new Query(Criteria.where("_id").is(t.getId())), update, getClazz(), dbName).getModifiedCount();
    }
